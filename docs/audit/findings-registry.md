...
| F-V13-4 | 2026-06-04 | proposed | LOW | **线上眼验发现**：index.html CSP `default-src 'self'` 无 `font-src` → Vite 内联（<4096B）的 1 个 KaTeX 字体（data:font/woff2）被 CSP 拦截（线上 console error）。20 字体中 19 个 self-hosted 不受影响，仅该 1 个 family 回退系统字体（cosmetic，公式仍渲染）。v0.4.0 起存在，v1.4 眼验时surface。修：CSP 加 `font-src 'self' data:`（或 Vite `assetsInlineLimit:0` 禁内联）| 2026-06-04 v0.5.0 线上眼验 |
