...
| 2026-06-22 | v3.6 增量 audit（报告 `2026-06-22-v3.6-increment.md`）：文本高亮/标记 mark/ins（并入扩展链），**无 critical/high/medium / 无新 finding**。极小插件不破双实例同步；`<mark>/<ins>` 默认放行 + XSS 双引擎验；删除线 `~~` 核心不受影响。ac5-perf 负载 flake（隔离过）。unit 342 + e2e 197。第四批 scope 第二项 |
| 2026-06-22 | v3.7 增量 audit（报告 `2026-06-22-v3.7-increment.md`）：主题增强 5 档强调色（data-accent 正交 M6），**无 critical/high/medium / 无新 finding**。正交 M6（data-accent 独立 data-theme）+ 默认 blue 零变化（删属性用默认 --accent）+ anti-poisoning 复用 settings 范式；枚举校验无安全面。unit 347 + e2e 202。**第四批 scope 全交付** |
