...
- **模式描述**：在 jsdom + Vitest + Playwright + Solid 组合下，测试代码反复踩到平台 API 不完整 / 类型推断不稳 / 时序异步等陷阱。常见 4 类：
  ...
  4. **Playwright full 并行 suite page.goto 30s 超时 / 延迟膨胀**：根因 = **并行 worker CPU 竞争**（8 核 default workers 饱和），非 dev-server / webkit-mobile-UA（2026-06-03 重定性，commit `b840aeb`；换 vite preview 未解决，限 `workers:CI?1:2` + `navigationTimeout:60s` 才稳）。CI 稳因 workers:1。先前归因 mobile-safari / dev-server / Playwright 版本均被 confounded。**2026-06-05 v1.6 补**：新增 `browser.newContext`（移动端 devices）类测试 **加剧本地竞争**（额外 context 重）+ 三 webServer（dev+preview+pwa build）并行 → 偶发拖垮同批其他 test（E2E-v16-002 full run 失败 / 隔离 5/5 过 / 双引擎隔离过）。仍是本 #4 家族，CI workers:1 不受影响；e2e 不在 CI 门禁（仅本地）。缓解：重 context 测试 isolated 跑 / 或本地 workers:1 跑 full
  ...
  6. **线上眼验改完要破缓存，否则验到旧资源（与 #5 同源——"验证方式错→误判"）**：部署修复后线上眼验，浏览器 HTTP-cache 旧 `index.html` → 重新 `browser_navigate` 仍看到旧 console 报错 → 一度误判"修复没生效/没部署"。实测 `curl` 线上 HTML 已是新版。修：眼验破缓存——`browser_navigate('https://.../?cb=<变化值>')`（query 变化强制重取文档）/ 或 curl 比对线上资源确认部署到位再眼验。**2026-06-04 v0.5.0 CSP 修复复验时踩**：cache-bust 后 console 才归零。**6b（PWA 加层 / 2026-06-05 v0.8.0 踩）：自 v0.6.0 上 PWA 后，`?cb=` 不够——Service Worker precache 是 cache-first，query 绕不过 SW 缓存层，眼验会看到上一版整个 app**（v0.8.0 眼验先看到 v0.7.0：clientH 不 respect 新布局 + 无 source-line，一度误判新版没生效）。PWA 破缓存必须**额外破 SW 层**：`(await navigator.serviceWorker.getRegistrations()).forEach(r=>r.unregister())` + `(await caches.keys()).forEach(k=>caches.delete(k))`，再 reload。判据：项目装了 SW（M8）→ 眼验破缓存默认含 SW 层，不只 `?cb=`
  ...
  8. **SW/Cache 类功能眼验：工具切不了 offline 时用"三证据链"替代，别假装验过也别跳过**：v0.6.0 PWA 眼验时 Playwright MCP 未暴露 `setOffline`（网络 toggle 需 CDP/context 能力）→ 无法在真浏览器亲跑"线上断网 reload"。应对：不谎称"验过离线"，改用**三证据链**——① SW 已 `controlling` + scope 正确 ② `caches.keys()`/`cache.keys()` 实测离线必需资产已填充 ③ 同一份构建（preview=线上 dist）的 e2e 已跑通 `context.setOffline`。**并验 cache 分布是否符合设计**（v0.6.0：precache 47 + mermaid-chunks 4 = 印证 F-V15-1 的 cache-on-use 架构）。诚实标注"断网 reload 本身是推断，非真浏览器亲跑"
  9. **依赖"状态变迁触发"的功能（SW 更新提示）单次部署眼验验不了，需跨部署或标 gap**：SW「有新版→提示刷新」要 SW 进 waiting 才触发，单次部署的眼验造不出"新版"→ 该链路眼验天然验不到（v0.6.0 AC-v15-4 仅单测 mock，F-V15-3）。修：要么后续任意一次部署后顺带眼验更新提示，要么显式标记为"跨部署才能验"的已知 gap，不在单次眼验里假装覆盖
...
- **remediation**：
  ...
  - **线上眼验破缓存（#6）**：部署后眼验加 cache-bust（`?cb=<变化值>`）或先 curl 比对线上资源确认部署到位，再下"修没生效"结论。"重新导航仍报旧错"先怀疑浏览器 HTTP-cache，别先归因部署失败。**PWA 项目（装了 SW / M8）额外破 SW 层（#6b）**：`?cb=` 绕不过 SW precache（cache-first）→ 必须 `serviceWorker.getRegistrations()→unregister()` + `caches.keys()→delete()` 再 reload，否则看到上一版整个 app
  - **眼验 console 干净（#7）**：眼验固定 checklist 加一步"console error/warning 截零"——功能渲染 ✓ 不代表 console 干净；红字即便不阻断功能也要定性（记 finding 或当场修），不许"看着能用就过"
  - **SW/Cache 眼验三证据链（#8）**：眼验工具切不了 offline 时，用 SW controlling + caches 实测填充 + 同构 e2e 三证据替代，并验 cache 分布符合设计；诚实标"断网 reload 是推断非亲跑"，不谎称验过、不跳过
  - **状态变迁功能跨部署验（#9）**：SW 更新提示等"需状态变迁才触发"的功能，单次部署眼验验不到 → 后续部署顺带验，或显式标"跨部署 gap"，不假装单次覆盖
- **实例**：
  ...
  - 2026-06-05 v0.6.0 PWA（#8）：MCP 无 setOffline → 用 SW controlling + caches 实测（precache 47 + mermaid-chunks 4）+ 同构 e2e 三证据验离线，诚实标"断网 reload 推断非亲跑"
  - 2026-06-05 v0.8.0 滚动同步（#6b）：眼验先看到 v0.7.0（SW precache cache-first，`?cb=` 绕不过）→ clientH 不 respect 新布局 + dslCount 0，一度误判新版没生效 → unregister SW + caches.delete 后才拿到 v0.8.0（面板内部滚动 + source-line 对齐 0px）
  - 2026-06-05 v0.6.0 PWA（#9）：SW 更新提示单次部署验不到（需 SW waiting）→ 标 F-V15-3 gap，留后续部署顺带验
- **跨项目升级路径**：升级为 standard testing setup template（vitest + playwright + jsdom）的"已知陷阱"段 → FB 候选（trap #4 worker 竞争 + #5 hash-routing 冷加载 + **#6 眼验破缓存 + #7 眼验 console 干净 + #8 SW/Cache 三证据链 + #9 状态变迁跨部署验**跨项目通用，优先上报；#5/#6/#7/#8/#9 同属"线上眼验认知陷阱→误判/漏验"家族，合并入 FB-006）
