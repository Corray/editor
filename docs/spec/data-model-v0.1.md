# 数据模型 v0.1 — editor MVP

| 字段 | 值 |
|------|----|
| **状态** | `draft` (待评审 §8 [TBD] 后升 v1.0) |
| **版本** | v0.1 |
| **基线** | PRD v1.0 + 共识 v1.0 + 模块 v1.0 + 架构 v1.0 |
| **首版日期** | 2026-05-19 |
| **owner** | FE (Corray) |
| **配套文件** | `api-spec-v0.1.md` (接口设计) |
| **下游** | → 测试计划 → 代码 |
...
| # | 议题 | AI 倾向 | 评审 |
|---|------|--------|------|
| **TBD-D1** | localStorage value 编码：纯字符串 vs JSON wrapper | 纯字符串（MVP 简单，未来切 JSON 走 §7.2 迁移路径）| 待评审 |
| **TBD-D2** | 大文档 toast 阈值：1MB 是否合适 | 1MB（性能拐点经验值，可在评审改）| 待评审 |
| **TBD-D3** | ERROR 状态 5s 自动 fallback 到 IDLE 是否合理 | 合理（避免用户永久卡 ERROR；下次输入自然 retry）| 待评审 |
...
**评审通过后：** 升 v1.0 / commit / 进入下一节点 = **测试计划**（含本数据模型的状态机不变量测试）。
