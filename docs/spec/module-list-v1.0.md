...
|------|-------|-------|-------|-------|------|----------|----------|
| **M1 编辑** | 维护 Markdown 源文 + 输入交互；**〔v2.1〕+查找/替换 + 格式快捷键(B/I/K toggle) + 列表自动延续 + 字数统计；〔v2.4〕+Tab 缩进；〔v2.7〕+格式工具栏(8 按钮，移动端格式入口)** | §3 / §4 (隐) + 共识 v2.1/v2.4/v2.7 | F1 | FE | in-dev (#6 + v2.1/v2.4/v2.7 编辑增强) | 〔v2.1〕`document.execCommand('insertText')`（undo 保持，ADR-017）| 〔v2.1〕chrome 文案 ← M7；status bar 被 M5 容纳；〔v2.7〕FormatToolbar 在 EditorArea 内 |
