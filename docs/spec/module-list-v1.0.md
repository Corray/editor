...
|------|-------|-------|-------|-------|------|----------|----------|
| **M1 编辑** | 维护 Markdown 源文 + 输入交互；**〔v2.1〕+查找/替换 + 格式快捷键(B/I/K toggle) + 列表自动延续 + 字数统计；〔v2.4〕+Tab 缩进；〔v2.7〕+格式工具栏(8 按钮，移动端格式入口)；〔v2.8〕+表格插入+Tab 单元格导航；〔v3.2〕+文档统计面板（点 status bar 展开）** | §3 / §4 (隐) + 共识 v2.1/v2.4/v2.7/v2.8/v3.2 | F1 | FE | in-dev (#6 + v2.1/v2.4/v2.7 编辑增强) | 〔v2.1〕`document.execCommand('insertText')`（undo 保持，ADR-017）| 〔v2.1〕chrome 文案 ← M7；status bar 被 M5 容纳；〔v2.7〕FormatToolbar 在 EditorArea 内 |
| **M2 预览** | 实时渲染 Markdown 为 HTML；**〔v2.3〕+fenced code 语法高亮（hljs 懒加载，不放宽 sanitize）** | §4.2 + 共识 v2.3 | F2 | FE | in-dev (#1 pipeline + #8 PreviewArea 挂载；v2.3 highlight) | sanitize 库 DOMPurify v3.4.5（ADR-002 accepted）、Markdown 渲染库 markdown-it v14.1.1（ADR-001 accepted）、〔v2.3〕highlight.js 11.11.1 lib/common 懒加载（ADR-019）；**〔v3.1〕+task list 可点 checkbox 渲染 + 点击回写源文（ADR-027）；〔v3.3〕+frontmatter 识别 + metadata 框（ADR-029）；〔v3.4〕+emoji/脚注/上下标懒加载（ADR-030）；〔v3.5〕+callout 容器块（ADR-031）；〔v3.6〕+mark/ins 高亮标记（ADR-032）** | ← M1；〔v3.1〕点击回写经 state.setText |
...
| **M13 设置**〔v2.9 新增〕| 用户设置（快照间隔/上限可调）+ localStorage 持久化；收口散落常量单一来源；承载 v3.0 语言切换；**〔v3.7〕+强调色预设（data-accent，ADR-033）** | 共识 v2.9 | PM 新拍 scope | FE | proposed (v2.9) | localStorage | SettingsDialog 由 AppShell 装配；→ M9（manager 读 settings 决定快照间隔/上限）；M1 prefs / M6 theme 持久化同范式 |
