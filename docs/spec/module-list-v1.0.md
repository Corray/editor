...
| **M13 设置**〔v2.9 新增〕| 用户设置（快照间隔/上限可调）+ localStorage 持久化；收口散落常量单一来源；承载 v3.0 语言切换 | 共识 v2.9 | PM 新拍 scope | FE | proposed (v2.9) | localStorage | SettingsDialog 由 AppShell 装配；→ M9（manager 读 settings 决定快照间隔/上限）；M1 prefs / M6 theme 持久化同范式 |
