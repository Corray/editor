# 测试计划 v0.1 — editor MVP

| 字段 | 值 |
|------|----|
| **状态** | `draft` (待评审 §9 [TBD] 后升 v1.0) |
| **版本** | v0.1 |
...
| # | 议题 | AI 倾向 | 评审 |
|---|------|--------|------|
| **TBD-T1** | Lighthouse CI 是否 MVP 上 | 不上，手工跑（MVP simplify，自动化属 v1.1+）| 待评审 |
| **TBD-T2** | E2E 浏览器矩阵：仅 Chromium vs 加 Webkit + Firefox | 仅 Chromium + Webkit（移动 Safari 是 PRD 必测项，Firefox 留 v1.1+）| 待评审 |
| **TBD-T3** | E2E 触发时机：每 PR vs 仅 main push | 仅 main push（PR 仅单测，省 CI 时间）| 待评审 |
| **TBD-T4** | 测试数据 fixtures 位置 | `tests/fixtures/`（统一目录，单测 / E2E 共享）| 待评审 |
...
**评审通过后：** 升 v1.0 / commit / 进入下一节点 = **代码实现**（按本计划执行测试，不等代码完成再补）。
